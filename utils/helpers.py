...
def log_activity(folder_id, detail_id, header_id, activity_type, status, message, user):
    ...
    session.sql(f"""
        INSERT INTO {DB}.FILE_ACTIVITY_LOG
            (HEADER_ID, FOLDER_ID, DETAIL_ID,
             ACTIVITY_TYPE, ACTIVITY_STATUS, ACTIVITY_MESSAGE, EXECUTED_BY)
        VALUES ({h}, {fo}, {d},
                '{activity_type}', '{status}', $${message}$$, '{user}')
    """).collect()


def log_bulk(folder_id: int, header_id: int, detail_ids: list,
             activity_type: str, message: str, user: str):
    for did in detail_ids:
        log_activity(folder_id, did, header_id, activity_type, "SUCCESS", message, user)